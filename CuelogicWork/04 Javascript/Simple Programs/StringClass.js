/*
String class
String in typescript is considered as sequance of characters.
There are multiple methods of string class that we can use while writting application.
*/
// Create object of string class as
var str = new String("Pallavi");
var ptr = new String("Deshmukh");


// charAt function gives character at specific index.
console.log("str.charAt(0) is:" + str.charAt(0));
console.log("str.charAt(1) is:" + str.charAt(1));


// charCodeAt gives unicode result
console.log("str.charCodeAt(0) is:" + str.charCodeAt(0));
console.log("str.charCodeAt(1) is:" + str.charCodeAt(1));
